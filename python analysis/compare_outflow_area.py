...
def isolate_cp(cold_pool, n_cluster=100, xthresh=20, ythresh=250):
    ...
    retained_cp = np.full_like(cold_pool, np.nan)
...
def cp_data(run,ds0,f,cx_in,cy_in):
    ...
    neg_count = np.count_nonzero(~np.isnan(cp_isolated))
        ...
        if os.path.isdir(os.path.join(indir,folder)) and folder in runs:
            ...
            for file in sorted(os.listdir(run)):
                ...
                and (sf <= int(file[-6:-3]) <= ef):
                    ...
                    neg_area,time = cp_data(run,ds0,f,cx,cy)
                        
                    neg_tseries.append(neg_area.flatten())
                    all_data.append(neg_area.flatten())
                    times.append(time)
            ...
            # area_concat = np.concatenate(neg_tseries)
            
            # cp_area_median = np.round(np.nanpercentile(area_concat,50),4)
            
            # csv_data = [[sim_name+mem,folder,cp_area_median]]
            
            # csv_path = '/t1/topping/point_data/median_cp_area_points.csv'
            # with open(csv_path, 'a', newline='') as file:
            #     writer = csv.writer(file)
            #     writer.writerows(csv_data)
            
            # cp_area_mean = np.round(np.nanmean(area_concat),4)
            
            # csv_data = [[sim_name+mem,folder,cp_area_mean]]
            
            # csv_path = '/strm4/topping/revised_figs/mean_cp_area_points.csv'
            # with open(csv_path, 'a', newline='') as file:
            #     writer = csv.writer(file)
            #     writer.writerows(csv_data)
...
axs.set_ylabel('Cold pool area ($km^2$)', fontsize=18)
...
# if int(member) != 0:
#     plt.savefig(outpath+'pdf/new/'+'member'+member+ '_ts_cp_area'+'.pdf',bbox_inches='tight')
#     plt.savefig(outpath+'png/new/'+'member'+member+ '_ts_cp_area'+'.png',dpi=300,bbox_inches='tight')
# else:
#     plt.savefig(outpath+'pdf/new/'+CAPE + '_' + HODO + '_' + MP + '_ts_cp_area'+'.pdf',bbox_inches='tight')
#     plt.savefig(outpath+'png/new/'+CAPE + '_' + HODO + '_' + MP + '_ts_cp_area'+'.png',dpi=300,bbox_inches='tight')

outpath = '/strm4/topping/revised_figs/cp_area_2500.pdf'
